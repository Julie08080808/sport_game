...
（commit `03836f6`）。**如果你能存取這個 git repo**，最簡單的方式是：
...
**如果你只有這份 `.md` 檔案、沒有 git 存取權限**，資料庫變更的完整 SQL 內容直接附在下面
（1.1 節），存成檔案跑就好；後端 Python 程式碼的部分請盡量想辦法拿到 git 存取權限直接 pull，
因為那 4 個檔案（`models/npc_model.py`、`models/game_model.py`、
`controllers/npc_controller.py`、`controllers/game_controller.py`）都是團隊共用、還在持續變動的檔案，
用 git 才能正確跟其他人的改動合併，用複製貼上的話風險很高。

### 1.1 資料庫變更（完整 SQL，不依賴 git，直接可跑）

把下面整段存成 `schema_tasks.sql`，執行 `psql -U postgres -d 你的資料庫名稱 -f schema_tasks.sql`。
全部用 `IF NOT EXISTS`／`WHERE NOT EXISTS` 寫法，在已經有資料、甚至已經跑過部分內容的資料庫上
重複執行也安全，不會刪資料或重複塞資料：
...
-- 顯示時直接用 scene_level 本身(最新已解鎖階段)。每次 scene_level 再往上升時會被清空(見 1.2 的
-- complete_task)，讓新解鎖的外觀重新變成預設，玩家可以再手動調回去──五棟建築各自獨立，不會一起變動。
...
-- ============================================================
-- 9. 示範資料：一個 NPC + 三個任務(手/腳/壓力球)，方便實測整條路
-- ============================================================
...
> 這份 SQL 除了本篇「建築外觀升級」需要的 `completion_count`/`display_level_cap` 兩個欄位（開頭那段）
> 之外，也包含了整套 NPC 任務系統的表(`npcs`/`tasks`/...)——因為 `/api/npc/task/complete` 這支 API
> 是建立在這套任務系統上的，兩者密不可分，缺任何一張表都會直接報錯。

### 1.2 後端 Python 程式碼

這 4 個檔案改動的完整內容在 git commit `03836f6` 跟 `729ccf0`／`68c9a74`（NPC 任務系統本體）裡，
**強烈建議直接 `git pull` 取得**，不要手動複製貼上——這些檔案還在被團隊持續修改，手動貼容易蓋掉
別人正在做的東西。如果真的完全沒有 git 存取權限，回頭找原本傳這份 `.md` 檔案的人要這 4 個檔案的
完整內容：`recipe-app/models/npc_model.py`、`recipe-app/models/game_model.py`、
`recipe-app/controllers/npc_controller.py`、`recipe-app/controllers/game_controller.py`。
