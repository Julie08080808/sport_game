...
// 從網址讀取玩家 user_id(Unity WebView 開這個頁面時會帶上,例如 /quiz?user_id=5)
// 沒有帶(例如純網頁訪客)就不發獎勵,答題功能本身照常可用
...
async function showResult() {
    ...
    if (correctCount === 0) {
        document.getElementById('score-comment').textContent = '加油,未獲得經驗值';
        return;
    }

    // 沒有 user_id(例如純網頁訪客),不呼叫獎勵 API,只顯示答對題數
    if (!USER_ID) {
        document.getElementById('score-comment').textContent = `答對 ${correctCount} 題`;
        return;
    }

    document.getElementById('score-comment').textContent = '正在發放獎勵...';

    try {
        const res = await fetch(`${API_QUIZ}/reward`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `user_id=${encodeURIComponent(USER_ID)}&correct_count=${correctCount}`
        });

        if (!res.ok) {
            document.getElementById('score-comment').textContent = `答對 ${correctCount} 題,獎勵發放失敗`;
            return;
        }

        const data = await res.json();
        document.getElementById('score-comment').textContent =
            `恭喜答對 ${correctCount} 題,獲得經驗值 +${data.exp_gained}、金幣 +${data.coin_gained}`;
    } catch (err) {
        document.getElementById('score-comment').textContent = `答對 ${correctCount} 題,獎勵發放失敗(連線問題)`;
    }
}
